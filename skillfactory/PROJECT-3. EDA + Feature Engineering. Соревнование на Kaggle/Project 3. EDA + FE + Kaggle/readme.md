...
<h2 style="text-align: center;"> Описание проблемы </h2>
...
<div align="center"> ### Описание датасета </div>
<div align="center"> #### Признаки, которые есть в датасете </div>
...
<div align="center"> ### Этапы работы над проектом </div>
...
Для оценки качества модели — точности прогнозов, сделанных моделью, — мы будем использовать метрику (некий числовой показатель), которая называется MAPE (mean absolute percentage error), средняя абсолютная процентная ошибка. Эту метрику очень легко интерпретировать.
 ...
 <div align="center">  $$MAPE = \frac{{1}}{n} \sum\limits_{i=1}^n\frac{y_{true_i} - y_{pred_i}}{y_{true_i}} * 100\% $$ </div>

<font size="10"> где $y_{true_i}$ — фактические значения прогноза, a $y_{pred_i}$ — предсказанные.  </font>  
